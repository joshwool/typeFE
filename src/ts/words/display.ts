...
export function wordsFocus(): void {
    $("#wordDis").on("click",() => { // Sets up an event listener to wait for a click on the word display
...
export function updateLines(): void {
    ...
    if (lineInd > 2) { // Checks if the line index is greater than 2
...
export function resizeLines(): void {
    $(".line").each((i, line) => {
        ...
        if (lineWidth > $("#wordDis").outerWidth()) { // Checks if line width is greater than display width
            console.log("shrink");
            while (lineWidth > $("#wordDis").outerWidth()) {
                lineWidth -= $(line).children().last().outerWidth(true);
                if ($(".line").eq(i+1).length === 0) { // Checks if the next line exists
                    $("#buffer").prepend($(line).children().last()); // Prepends word to buffer if next line doesn't exist
                }
                else {
                    $(".line").eq(i+1).prepend($(line).children().last()); // Prepends word to buffer if next line does exist
                }
            }
        }
            ...
            while (underflow < 0) {
                ...
                while (pullElement === undefined) { // Checks if next line has words to pull from
                    if ($(".line").eq(pull).children().length > 0) { // Checks if there are words in the line
                        pullElement = $(".line").eq(pull);
                    }
                    else if (pull < $(".line").length-1) { // Checks if all lines have been iterated through
                        pull++;
                    }
                    else {
                ...
                underflow += (pullElement.children().first().children().length * 14.41 + 8); // Calcs width of word to be pulled and adds to underflow
                if (underflow < 0) {
                    if (pullElement.children().length === 0) { // Checks if there are any words in the pull element
                    ...
                    else { // Appends the first word in the pull element to the line
...
export function charBack(): void {
    ...
    if (inpLength > gen.wordList[input.wordInd].length) {
        ...
        $(".wordCur").children().eq(inpLength-1).removeClass("correct incorrect");
    }
}
